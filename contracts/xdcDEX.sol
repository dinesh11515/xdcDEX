...
contract  xdcDEX {
    ...
    function sellXdc(uint256 amount,uint256 price) public payable {
        require(msg.value == amount, "You must send the exact amount");
        if(sellerList[msg.sender].seller != msg.sender){
            sellerList[msg.sender] = SellerList(payable(msg.sender), address(0),"XDC",true,listId, amount,0, price, block.timestamp);
    ...
    function buyXdcRequest(uint256 id,uint256 amount,string memory _name) public payable {
        ...
        buyRequests[seller].push(buyRequest(payable(msg.sender),seller,address(0),true,false,true,false,false,amount,listings[id].price,orderId,_name,"XDC"));
        ...
        emit request(seller,_name,"XDC",amount,listings[id].price,orderId);
        orderId++;
    }
